...
export default function LoadingScreen({ onLoadingComplete }) {
  ...
  // 1. Smooth Progress & Transition Handler (5 Seconds)
  useEffect(() => {
  ...
  // 2. 60 FPS 3D Environment & Atmospheric Canvas Engine
  useEffect(() => {
    ...
    const draw3DSymbols = (t) => {
      ...
      const pulseFactor = Math.sin(t * 0.003) * 0.15 + 1;

      // Symbol 1: Pink Neon CIRCLE (Left)
      const circleX = width * (isMobile ? 0.18 : 0.22);
      const circleY = centerY + Math.sin(t * 0.002) * 14;
      const circleRadius = (isMobile ? 38 : 65) * pulseFactor;
      ...
      const triY = centerY - (isMobile ? 75 : 120) + Math.cos(t * 0.002) * 12;
      const triSize = (isMobile ? 42 : 70) * pulseFactor;
      ...
      const sqY = centerY + Math.sin(t * 0.0025 + 1) * 14;
      const sqSize = (isMobile ? 36 : 60) * pulseFactor;

      ctx.save();
      ctx.translate(sqX, sqY);
      ctx.rotate(Math.sin(t * 0.0015) * 0.12);
      ctx.beginPath();
    ...
    const drawReflectiveFloor = (t) => {
      ...
      pinkGlow.addColorStop(0, `rgba(244, 63, 94, ${0.18 + Math.sin(t * 0.003) * 0.05})`);
      ...
      cyanGlow.addColorStop(0, `rgba(56, 189, 248, ${0.18 + Math.cos(t * 0.003) * 0.05})`);
