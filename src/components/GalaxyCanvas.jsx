...
export default function GalaxyCanvas() {
  ...
  useEffect(() => {
    ...
    // Mobile performance particle scaling
    const particleFactor = tier === 'low' ? 0.2 : tier === 'medium' ? 0.35 : 1.0;

    // 1. Arcane Energy Vortex Group
    ...
    const cSilver = new THREE.Color('#ffffff');
    const cEmerald = new THREE.Color('#ef4444');
    ...
    const cTitanium = new THREE.Color('#ffe4e6');
    const cMint = new THREE.Color('#f87171');
    const cDoomGreen = new THREE.Color('#b91c1c');
    ...
    const armMat = new THREE.PointsMaterial({
      size: isMobile ? 4.5 : 6.2, map: starTexture, vertexColors: true, transparent: true, opacity: 0.92, depthWrite: false, blending: THREE.AdditiveBlending
    });
    ...
    const cGold = new THREE.Color('#ffffff');
    ...
    const nebMat = new THREE.PointsMaterial({
      size: isMobile ? 8 : 11, map: nebulaTexture, vertexColors: true, transparent: true, opacity: 0.45, depthWrite: false, blending: THREE.AdditiveBlending
    });
    ...
    const tunnelMat = new THREE.PointsMaterial({
      size: isMobile ? 3 : 4.2, map: starTexture, vertexColors: true, transparent: true, opacity: 0.88, blending: THREE.AdditiveBlending
    });
    ...
    const animate = () => {
      ...
      coreMat.size = (isMobile ? 5 : 7.5) + Math.sin(elapsedTime * 1.8) * 0.6;
