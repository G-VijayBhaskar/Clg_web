...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 right-0 z-[60] bg-[#0c0408]/95 border-b border-pink-500/40 py-2 overflow-hidden shadow-[0_4px_20px_rgba(244,63,94,0.25)] backdrop-blur-xl w-full max-w-full">
      ...
      <header className={`navbar mt-[38px] w-full max-w-full ${scrolled ? 'scrolled' : ''}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between pt-1 w-full min-w-0">
          <a href="#home" className="flex items-center gap-2.5 sm:gap-3.5 group min-w-0 flex-1 sm:flex-initial mr-2">
            <div className="w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 shrink-0 rounded-2xl bg-black p-1 border-2 border-pink-500/70 flex items-center justify-center group-hover:border-sky-400 transition-all shadow-[0_0_20px_rgba(244,63,94,0.5)] overflow-hidden">
              <img src="/teciton_logo.webp" alt="TECITON Logo" decoding="async" className="w-full h-full object-cover rounded-xl" />
            </div>
            <div className="flex flex-col min-w-0">
              <span className="font-serif text-xl sm:text-2xl md:text-3xl font-normal text-white tracking-wide truncate">
                TECITON <span className="italic text-pink-400 font-serif">2026</span>
              </span>
              <span className="text-[10px] sm:text-xs md:text-base text-sky-300 uppercase tracking-wider sm:tracking-widest font-extrabold font-mono truncate">
        ...
        {/* Mobile Menu Drawer */}
        {mobileMenuOpen && (
          <div className="md:hidden bg-[#0e0409]/95 border-b border-rose-500/30 px-4 sm:px-6 py-6 mt-3 backdrop-blur-xl flex flex-col gap-4 w-full max-w-full overflow-hidden">
            <a
              href="#home"
